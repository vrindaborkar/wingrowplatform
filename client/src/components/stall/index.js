...
const StallComponent = props => {
  ...
  const handleStallClick = (row, col, data) => {
    ...
    // Now update the bookStalls state
    const existingStalls = [...bookStalls]

    // Update only the relevant market and date stalls
    ...
    // If no matching market and date, add a new group of stalls
    ...
    // Calculate the total price for each market/date combination
    const finalUpdatedStalls = updatedStalls.map(marketStall => {
      if (marketStall.stalls && marketStall.stalls.length > 0) {
        ...
        // If no stalls are selected, set total price to 0
