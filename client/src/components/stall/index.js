...
const StallComponent = (props) => {
  ...
  //   const updatedStalls = selectedStallsRedux.map((market) => ({
  //     ...market,
  //     stalls: market.stalls.filter((stall) => stall.id !== stallId),
  //   }));
  //   dispatch({
  //     type: "UPDATE_SELECTED_STALLS",
  //     payload: updatedStalls,
  //   });
  // };
  ...
  const handleRemoveStall = (stallId) => {
    ...
    if (isStallSelected) {
      ...
      setTotalPrice((prevPrice) => prevPrice - (stallDataMap.get(stallId)?.stallPrice || 0));
    ...
    setSelectedStallsMap(newSelectedStalls);  
    const groupedStalls = Object.keys(newSelectedStalls).flatMap((marketName) => {
      const marketStalls = newSelectedStalls[marketName];
      return Object.keys(marketStalls).map((date) => {
        const dateStalls = marketStalls[date].map((stallId) => {
          const stall = stallDataMap.get(stallId);
          return {
            id: stall ? stall.id : "",
    ...
    if (JSON.stringify(groupedStalls) !== JSON.stringify(bookStalls)) {
      setbookStalls(groupedStalls);
        ...
        {loading ? (
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="stall-form">
            ...
            <div className="border-green-200 border-1 p-2">
              ...
              <hr />
              <div
                className={`market-container ${roadPosition === "top" ? "row p-3" : "column"
                  }`}
              >
                ...
                <div
                  className={`market-layout w-100 ${roadPosition === "left" ? "pl-1" : ""
                    }`}
                  style={{
                  ...
                  {stallPositions.map((row, rowIndex) =>
                    row.map((isStall, colIndex) => {
                      ...
                      return (
                        <div
                          key={stallId}
                          className={`stall ${isDisabled ? "disabled-stall" : ""
                            } ${isStall.value
