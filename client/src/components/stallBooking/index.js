...
const Stall = ({ x, y, width, height, label }) => (
  <>
    <Rect
      ...
      height={height}
      stroke="black"
      fill="#88c0d0" // Lighter fill color for visibility
      strokeWidth={2}
      cornerRadius={5} // Rounded corners for better aesthetics
    />
    {label && (
      <Text
        x={x + 5}
        ...
        text={label}
        fontSize={14}
        fontStyle="bold" // Bold font for stall labels
        fill="black"
      />
    )}
  </>
    ...
    <Stage width={window.innerWidth} height={window.innerHeight}>
      <Layer>
        ...
        <Line
          ...
          strokeWidth={2}
        />
        <Circle x={600} y={300} radius={50} stroke="black" strokeWidth={2} fill="#FFD700" />
        <Text x={600 - 20} y={300 - 10} text="Center" fontSize={14} fontStyle="bold" fill="black" />
        {stalls.map((stall, index) => (
          <Stall key={index} {...stall} />
        ))}
        <Text x={50} y={20} text="Welcome to the Fruit Market!" fontSize={24} fontStyle="bold" fill="green" />
      </Layer>
    </Stage>
  );
};
